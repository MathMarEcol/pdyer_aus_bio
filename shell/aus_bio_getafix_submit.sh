...
# Example use:
# GIT_BRANCH=f_targets_debug WORKER_SMALL_N=20 WORKER_SMALL_MEM=20GB WORKER_BIG_N=1 WORKER_BIG_MEM=100GB WORKER_RUNTIME=7-00:00 sbatch aus_bio_getafix_submit.sh
set -euo pipefail
...
if [[ -v WORKER_SMALL_N ]]
then
   export WORKER_SMALL_N
fi
if [[ -v WORKER_BIG_N ]]
then
   export WORKER_BIG_N
fi
if [[ -v WORKER_SMALL_MEM ]]
then
   export WORKER_SMALL_MEM
fi
if [[ -v WORKER_BIG_MEM ]]
then
   export WORKER_BIG_MEM
fi
if [[ -v WORKER_MEM ]]
then
   export WORKER_MEM
