...
export SCHEDULER="multiprocess" #note, this is clustermq multiprocess (callr) not future multiprocess which deprecated and gives sequential behaviour
export R_FUTURE_GLOBALS_MAXSIZE=${R_FUTURE_GLOBALS_MAXSIZE:-100000000000}
export TF_FORCE_GPU_ALLOW_GROWTH="true"
export TENSOR_CPU_MEM_MAX=${TENSOR_CPU_MEM_MAX:-50000000000} #Certain operations that do bulk operations over matricies will batch to keep RAM usage within this amount (in bytes)
export TENSOR_GPU_MEM_MAX=${TENSOR_GPU_MEM_MAX:-4500000000} #Certain operations that do bulk operations over matricies will batch to keep GPU memory usage within this amount (in bytes)
