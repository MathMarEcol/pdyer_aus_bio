...
rsync -irc $ROOT_STORE_DIR/Q1215/mpa_poly_june_2023/ $TMP_DATA_DIR/mpa_poly_june_2023
...
then
   ...
   ## Breaking the pipeline up into stages. see aus_bio_getafix_submit.sh for details
    ...
    Rscript -e "targets::tar_make_clustermq(c(gfbootstrap_cluster), workers = ${WORKERS}, log_worker = TRUE)"
		...
		Rscript -e "targets::tar_make_clustermq(c(starts_with('cluster_env_assign_cluster_')), workers = 1, log_worker = TRUE)"

		## S5 - Plotting can be memory intensive
		Rscript -e "targets::tar_make_clustermq(c(gfbootstrap_plotted, starts_with('extrap_plotted_')), workers = 1, log_worker = TRUE)"
